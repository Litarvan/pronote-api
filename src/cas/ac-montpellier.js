...
{
    console.log(`Logging in '${username}' for '${url}' using Montpellier CAS`);

    let jar = new jsdom.CookieJar();
    let dom = await start(url, jar);
    ...
    dom = await getDOM({
        url: `https://www.environnementnumeriquedetravail.fr/cas/login?service=${encodeURIComponent(url)}&idpId=2`,
        jar,
        asIs: true
    });

    //if (dom.indexOf('ATEN') !== -1)
...
async function start(url, jar, first = true)
{
...
function submitForm({ dom, jar, forLogin, followRedirects, asIs })
{
    ...
    if (url.indexOf('/') === -1)
    {
        url = 'https://famille.ac-montpellier.fr/login/' + url;
    }
